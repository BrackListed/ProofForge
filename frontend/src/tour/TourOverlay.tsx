...
export function TourOverlay() {
  const { isActive, stepIndex, steps, next, skip } = useTour();
  ...
  return (
    <div className="fixed inset-0 z-100 font-mono">
      {rect ? (
        <>
          <motion.div className="fixed left-0 right-0 top-0 bg-black/75" animate={{ height: Math.max(rect.top, 0) }} transition={spring} />
          <motion.div className="fixed left-0 right-0 bottom-0 bg-black/75" animate={{ top: rect.top + rect.height }} transition={spring} />
          <motion.div className="fixed left-0 bg-black/75" animate={{ top: rect.top, height: rect.height, width: Math.max(rect.left, 0) }} transition={spring} />
          <motion.div className="fixed right-0 bg-black/75" animate={{ top: rect.top, height: rect.height, left: rect.left + rect.width }} transition={spring} />
          <motion.div
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="fixed w-[380px] border border-indigo-500/30 bg-zinc-950/95 p-5 shadow-2xl shadow-black/60 backdrop-blur-xl"
          style={tooltipStyle}
        >
